...
class SpyTrainer_Sentences():
    def __init__(self, challenge, nb_inputs, nb_hidden, nb_outputs, time_step=1e-3, nb_steps=100, max_time=1.4,
                 batch_size=32, seed=None, dtype=torch.float):
    ...
    def sparse_datagen_from_hdf5(self, datafile, shuffle=True):
        ...
        for b in range(self.nb_batches):
            ...
            X_batch = torch.sparse.FloatTensor(i, v,
                                               torch.Size([self.batch_size, self.nb_steps + 1, self.nb_inputs])).to(
                self.device)
            y_batch = torch.tensor(y).to(self.device)
    ...
    def train(self, train_data, test_data=None, lr=1e-3, nb_epochs=10, checkpointPath=None):
        ...
        self.class_weights = torch.ones(self.nb_outputs).to(self.device)
        ...
        for e in range(nb_epochs):
            ...
            with tqdm(self.sparse_datagen_from_hdf5(train_data), unit="batch", leave=False) as tepoch:
                for x_local, y_local in tepoch:
                    ...
                    reg_loss += 2e-6 * torch.mean(
                        torch.sum(torch.sum(spks, dim=0), dim=0) ** 2)  # L2 loss on spikes per neuron

                    # Here we combine supervised loss and the regularizer
                    loss_val = loss_fn(log_p_y, y_local) + reg_loss
                    acc_val = np.mean((y_local[y_local != self.ignore_class] == am[
                        y_local != self.ignore_class]).detach().cpu().numpy())
                    optimizer.zero_grad()
                    loss_val.backward()
                    optimizer.step()
            ...
            if (e % 5 == 0 or e == (self.nb_epochs - 1)) and test_data is not None:
                ...
                if checkpointPath is not None:
                    ...
                    np.savez_compressed(os.path.join(checkpointPath, f"features_{e}"),
                                        w1=self.w1.detach().cpu().numpy(), w2=self.w2.detach().cpu().numpy(),
                                        v1=self.v1.detach().cpu().numpy())
                    print(f"saved{e}")
    ...
    def compute_classification_accuracy(self, datafile, plot_confusion_matrix=False):
        ...
        for x_local, y_local in self.sparse_datagen_from_hdf5(datafile, shuffle=False):
            ...
            _, am = torch.max(m, 1)  # argmax over output units
            tmp = np.mean(
                (y_local[y_local != self.ignore_class] == am[y_local != self.ignore_class]).detach().cpu().numpy())
            accs.append(tmp)
    ...
    def sparse_datagen_from_hdf5(self, datafile, shuffle=True):
       ...
       """
        ...
        for b in range(self.nb_batches):
            ...
            i = torch.LongTensor(coo).to(self.device)
            v = torch.FloatTensor(np.ones(len(coo[0]))).to(self.device)
            X_batch = torch.sparse.FloatTensor(i, v, torch.Size([self.batch_size, self.nb_steps, self.nb_inputs])).to(
                self.device)
